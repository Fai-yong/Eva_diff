...
def parse_args():
    ...
    parser.add_argument("--beta_dpo", type=float, default=5000,
...
def main():
    ...
    train_transforms = transforms.Compose(
        [
            ...
            transforms.RandomCrop(
                args.resolution) if args.random_crop else transforms.CenterCrop(args.resolution),
            transforms.Lambda(
                lambda x: x) if args.no_hflip else transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
    ...
    if args.train_method == 'dpo':
        ...
        def collate_fn(examples):
            ...
            # SDXL takes raw prompts
            if args.sdxl:
                return_d["caption"] = [example["caption"]
                                       for example in examples]
            else:
                return_d["input_ids"] = torch.stack(
        ...
        if args.choice_model:
            ...
            def choice_model_says_flip(batch):
                # Can switch to iteration but not needed for nwo
                assert len(batch['caption']) == 1
            ...
            if 'pickapic' in args.dataset_name:
                ...
                for im_0_bytes, im_1_bytes, label_0 in zip(examples['jpg_0'], examples['jpg_1'], examples['label_0']):
                    ...
                    images.append(Image.open(
                        io.BytesIO(im_bytes)).convert("RGB"))
            else:
    ...
    with accelerator.main_process_first():
        if 'pickapic' in args.dataset_name:
            ...
            if args.dreamlike_pairs_only:
                ...
                dataset[args.split] = dataset[args.split].select(
                    dream_like_idx)
                new_len = dataset[args.split].num_rows
                print(
        ...
        if args.max_train_samples is not None:
            dataset[args.split] = dataset[args.split].shuffle(
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_method == 'dpo':
                    ...
                    feed_pixel_values = torch.cat(
                        batch["pixel_values"].chunk(2, dim=1))
                ...
                with torch.no_grad():
                    latents = vae.encode(feed_pixel_values.to(
                ...
                if args.sdxl:
                    ...
                    if args.train_method == 'dpo':
                        prompt_batch["prompt_embeds"] = prompt_batch["prompt_embeds"].repeat(
                            2, 1, 1)
                        prompt_batch["pooled_prompt_embeds"] = prompt_batch["pooled_prompt_embeds"].repeat(
                            2, 1)
                ...
                if args.train_method == 'sft':  # SFT, casting for F.mse_loss
                    ...
                    model_losses = (
                        model_pred - target).pow(2).mean(dim=[1, 2, 3])
                    ...
                    with torch.no_grad():  # Get the reference policy (unet) prediction
                        ...
                        ref_losses = (
                            ref_pred - target).pow(2).mean(dim=[1, 2, 3])
                    ...
                    implicit_acc = (inside_term > 0).sum(
                    ).float() / inside_term.size(0)
                ...
                # - Implicit accuracy
                if args.train_method == 'dpo':
                    avg_model_mse = accelerator.gather(
                        raw_model_loss.repeat(args.train_batch_size)).mean().item()
                    avg_ref_mse = accelerator.gather(
                        raw_ref_loss.repeat(args.train_batch_size)).mean().item()
